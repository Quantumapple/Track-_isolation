...
workspace = "/cms/ldap_home/jongho/L1PixelTrigger/SignalWindow/Results/"
...
for i in range(1,number_of_cores+1):
    ...
    if not (os.path.exists(workdir)):
            ...
            cpscript = "cp ./sw_treeMaker.C ./sw_treeMaker.h ./x_sw_treeMaker.C " + output+ "Job_" + str(i) + "/."
            os.system(cpscript)
            set_each_path = "find " + output + "Job_" + str(i) + "/." + " -name 'sw_treeMaker.h' -type f -exec sed -i s/" + "txt_to_path/" + sample + "_%s" % (i) + ".txt" + "/g {} +"
            set_tree_temp_path = "find " + output + "Job_" + str(i) + "/." + " -name 'sw_treeMaker.h' -type f -exec sed -i s/" + "Tree_output/" + "Tree_" + sample + "_%s" % (i) + ".root" + "/g {} +"
            set_tree_output_path = "find " + output + "Job_" + str(i) + "/." + " -name 'x_sw_treeMaker.C' -type f -exec sed -i s/" + "Tree_output/" + "Tree_" + sample + "_%s" % (i) + ".root" + "/g {} +"
...
fr = open('./inputlist_SE_PU200_HGCAL_EGID.txt', 'r')
