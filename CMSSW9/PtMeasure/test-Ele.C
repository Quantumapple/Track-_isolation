...
void test::Loop(int nth_case)
{
   ...
   TH1F *hist_bsl1_l1l2_a = new TH1F("hist_bsl1_l1l2_a",";P_{T} (GeV); #Delta#phi", 4, 8, 16);   // 2 GeV
   TH1F *hist_bsl1_l1l2_b = new TH1F("hist_bsl1_l1l2_b",";P_{T} (GeV); #Delta#phi", 1., 16., 20.);  // 4 GeV
   TH1F *hist_bsl1_l1l2_c = new TH1F("hist_bsl1_l1l2_c",";P_{T} (GeV); #Delta#phi", 12, 20., 80.);   // 5 GeV
   TH1F *hist_bsl1_l1l2_d = new TH1F("hist_bsl1_l1l2_d",";P_{T} (GeV); #Delta#phi", 14, 80., 150.);   // 5 GeV
   TH1F *hist_bsl1_l1l2_e = new TH1F("hist_bsl1_l1l2_e",";P_{T} (GeV); #Delta#phi", 5, 150., 200.);   // 10 GeV
   ...
   points << "double gen_Pt[" << Num << "] ={}," << " point[" << Num << "] = {}," << " error[" << Num << "] = {};" << endl;
   points << "int points = " << Num << ";" << endl;
   ...
   for(Int_t nth = 0; nth < 4; nth++)
   {
       ...
       H[count]->GetXaxis()->SetRangeUser(H[count]->GetMean()-0.007, H[count]->GetMean()+0.007);
   ...
   } // end of nth loop ( 8 ~ 16 GeV )
 
   
   for(Int_t nth = 0; nth < 1; nth++)
   {
       cout << "Process: (" << count+1 << "/" << Num << ")" << endl;
       TString file_ = "./Plots/phi_";
       TString ith_;
       ...
       sprintf(histname, "hist_%dth", count+1);
       H[count] = new TH1F(histname,"",250,-0.1,0.1);
       ...
       func->SetParLimits(0,1e-4,10);                                          // const
       func->SetParLimits(1,H[count]->GetMean()-0.1,H[count]->GetMean()+0.1);  // mean
       func->SetParLimits(2,1e-5,0.1);                                         // sigma
   ...
   for(Int_t nth = 0; nth < 12; nth++)
   {
       ...
       sprintf(histname, "hist_%dth", count+1);
       H[count] = new TH1F(histname,"",1000,-0.1,0.1);
       ...
       func->SetParameters(1, H[count]->GetMean(), 0.001);
      
       // Set fit parameter limits
       func->SetParLimits(0,1e-4,0.1);                                         // const
       func->SetParLimits(1,H[count]->GetMean()-0.1,H[count]->GetMean()+0.1);  // mean
       func->SetParLimits(2,1e-5,0.1);                                         // sigma
   ...
   } // end of nth loop ( 20 ~ 80 GeV )

   
   for(Int_t nth = 0; nth < 14; nth++)
   {
       cout << "Process: (" << count+1 << "/" << Num << ")" << endl;
       TString file_ = "./Plots/phi_";
       TString ith_;
       ...
       // Set fit parameter limits
       func->SetParLimits(0,1e-4,0.5);                                         // const
       func->SetParLimits(1,H[count]->GetMean()-0.1,H[count]->GetMean()+0.1);  // mean
       func->SetParLimits(2,1e-5,0.1);                                         // sigma
   ...
   } // end of nth loop ( 80 ~ 150 GeV )
  
         
   for(Int_t nth = 0; nth < 5; nth++)
   {
       cout << "Process: (" << count+1 << "/" << Num << ")" << endl;
       TString file_ = "./Plots/phi_";
       TString ith_;
   ...
   Int_t nbins = 200; Float_t x1 = 0.; Float_t x2 = 200.;
