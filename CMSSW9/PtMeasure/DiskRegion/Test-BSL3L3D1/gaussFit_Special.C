#define gaussFit_Special_cxx
#include "gaussFit_Special.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void gaussFit_Special::Loop()
{
   if (fChain == 0) return;
   ...
   TH1F *hist_a = new TH1F("hist_b",";P_{T} (GeV); #Delta#phi", 10, 2., 6.);     // 0.4 GeV
   TH1F *hist_b = new TH1F("hist_c",";P_{T} (GeV); #Delta#phi", 4, 6., 8.);      // 0.5 GeV
   TH1F *hist_c = new TH1F("hist_d",";P_{T} (GeV); #Delta#phi", 12, 8., 20.);    // 1 GeV
   TH1F *hist_d = new TH1F("hist_e",";P_{T} (GeV); #Delta#phi", 15, 20., 50.);   // 2 GeV
   TH1F *hist_e = new TH1F("hist_f",";P_{T} (GeV); #Delta#phi", 10, 50., 100.);  // 5 GeV
   ...
   for(Int_t nth = 0; nth < 10; nth++)
   {
	   ...
	   Int_t size = genpT_bsl3_l3d1->size();
	   if( size == 0 ) continue;

       Float_t gen_pT = genpT_bsl3_l3d1->at(0);
	   if( gen_pT > high_pt || gen_pT < low_pt ) continue;

       Float_t dphi = bsl3_l3d1_dphi->at(0);
   ...
   */
   ...
   for(Int_t nth = 0; nth < 10; nth++)
   {
       ...
       H[count]->GetXaxis()->SetRangeUser(H[count]->GetMean()-0.0015, H[count]->GetMean()+0.0015);
       func->SetParameters(70, H[count]->GetMean(), 0.001);
