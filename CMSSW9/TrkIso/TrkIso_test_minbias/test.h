...
public :
   ...
   void TrkIso_L123(float recoPV);
   void TrkIso_L124(float recoPV);
   void TrkIso_L134(float recoPV);
   void TrkIso_L234(float recoPV);
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      ...
      string line;
      ifstream myfile("MinBias_PU200_1.txt"); // txt_to_path will be replaced by the name of txt file that contains the location of input files
      if (myfile.is_open())
        {
...
void test::TrkIso_L123(float recoPV) {
    float dz_L13L12 = 0.0344;
    float dz_L13L13 = 0.0352;
    float dz_L13L23 = 0.0332;

    float dEta_pixelCut1 = 0.00266; // L12 - L13
    float dEta_pixelCut2 = 0.00446; // L12 - L23
    float dEta_pixelCut3 = 0.00178; // L13 - L23

    float dEta_PVCut1 = 0.00116; // PVL3 - PVL2
    float dEta_PVCut2 = 0.00260; // PVL3 - PVL1

    float ddphi_left = -0.00250;
    float ddphi_right = 0.00200;
            ...
            if( fabs(dz12) < dz_L13L12 )
            {
                for(std::vector<TVector3>::iterator c = L3.begin(); c != L3.end(); ++c)
                {
                    ...
                    if( fabs(dz13) < dz_L13L13 && fabs(dz23) < dz_L13L23 ) // Delta z cut
                        if( fabs(etaL2L3-etaL1L2) < dEta_pixelCut2 && fabs(etaL1L3-etaL1L2) < dEta_pixelCut1 && fabs(etaL2L3-etaL1L3) < dEta_pixelCut3  ) // Delta eta(pixel,pixel) cut
                            if( fabs(etaPVL3 - etaPVL2) < dEta_PVCut1 && fabs(etaPVL3 - etaPVL1) < dEta_PVCut2 ) // Delta eta(PV, pixel) cut
                                if( ( dPhi1 < 0 && dPhi2 < 0 ) || ( dPhi1 > 0 && dPhi2 > 0 ) ) // Delta phi sign cut
                                    if( cut > ddphi_left && cut < ddphi_right ) 
...
void test::TrkIso_L124(float recoPV) {
    float dz_L14L12 = 0.0336;
    float dz_L14L14 = 0.0332;
    float dz_L14L24 = 0.0348;

    float dEta_pixelCut1 = 0.00272; // L12 - L14
    float dEta_pixelCut2 = 0.00360; // L12 - L24
    float dEta_pixelCut3 = 0.00136; // L14 - L24

    float dEta_PVCut1 = 0.00090; // PVL4 - PVL2
    float dEta_PVCut2 = 0.00256; // PVL4 - PVL1

    float ddphi_left = -0.00500;
    float ddphi_right = 0.00150;
            ...
            if( fabs(dz12) < dz_L14L12 )
            {
                for(std::vector<TVector3>::iterator c = L4.begin(); c != L4.end(); ++c)
                {
                    ...
                    if( fabs(dz14) < dz_L14L14 && fabs(dz24) < dz_L14L24 ) // Delta z cut
                        if( fabs(etaL2L4-etaL1L2) < dEta_pixelCut2 && fabs(etaL1L4-etaL1L2) < dEta_pixelCut1 && fabs(etaL2L4-etaL1L4) < dEta_pixelCut3  ) // Delta eta(pixel,pixel) cut
                            if( fabs(etaPVL4 - etaPVL2) < dEta_PVCut1 && fabs(etaPVL4 - etaPVL1) < dEta_PVCut2 ) // Delta eta(PV, pixel) cut
                                if( ( dPhi1 < 0 && dPhi2 < 0 ) || ( dPhi1 > 0 && dPhi2 > 0 ) ) // Delta phi sign cut
                                    if( cut > ddphi_left && cut < ddphi_right ) 
...
void test::TrkIso_L134(float recoPV) {
    float dz_L14L13 = 0.0340;
    float dz_L14L14 = 0.0332;
    float dz_L14L34 = 0.0364;

    float dEta_pixelCut1 = 0.00132; // L13 - L14
    float dEta_pixelCut2 = 0.00274; // L13 - L34
    float dEta_pixelCut3 = 0.00262; // L14 - L34

    float dEta_PVCut1 = 2e-5;    // PVL4 - PVL3
    float dEta_PVCut2 = 0.00256; // PVL4 - PVL1
            ...
            if( fabs(dz13) < dz_L14L13 )
            {
                for(std::vector<TVector3>::iterator c = L4.begin(); c != L4.end(); ++c)
                {
                    ...
                    if( fabs(dz14) < dz_L14L14 && fabs(dz34) < dz_L14L34 ) // Delta z cut
                        if( fabs(etaL3L4-etaL1L3) < dEta_pixelCut2 && fabs(etaL1L4-etaL1L3) < dEta_pixelCut1 && fabs(etaL3L4-etaL1L4) < dEta_pixelCut3  ) // Delta eta(pixel,pixel) cut
                            if( fabs(etaPVL4 - etaPVL3) < dEta_PVCut1 && fabs(etaPVL4 - etaPVL1) < dEta_PVCut2 ) // Delta eta(PV, pixel) cut
                                if( ( dPhi1 < 0 && dPhi2 < 0 ) || ( dPhi1 > 0 && dPhi2 > 0 ) ) // Delta phi sign cut
                                    if( cut > ddphi_left && cut < ddphi_right ) 
...
void test::TrkIso_L234(float recoPV) {
    float dz_L24L23 = 0.0336;
    float dz_L24L24 = 0.0344;
    float dz_L24L34 = 0.0360;

    float dEta_pixelCut1 = 0.00176; // L23 - L24
    float dEta_pixelCut2 = 0.00356; // L23 - L34
    float dEta_pixelCut3 = 0.00224; // L24 - L34

    float dEta_PVCut1 = 0.00090; // PVL4 - PVL2
    float dEta_PVCut2 = 0.00256; // PVL4 - PVL1

    float ddphi_left = -0.00200;
    float ddphi_right = 0.00300;
            ...
            if( fabs(dz23) < dz_L24L23 )
            {
                for(std::vector<TVector3>::iterator c = L4.begin(); c != L4.end(); ++c)
                {
                    ...
                    if( fabs(dz24) < dz_L24L24 && fabs(dz34) < dz_L24L34 ) // Delta z cut
                        if( fabs(etaL3L4-etaL2L3) < dEta_pixelCut2 && fabs(etaL2L4-etaL2L3) < dEta_pixelCut1 && fabs(etaL3L4-etaL2L4) < dEta_pixelCut3  ) // Delta eta(pixel,pixel) cut
                            if( fabs(etaPVL4 - etaPVL3) < dEta_PVCut1 && fabs(etaPVL4 - etaPVL2) < dEta_PVCut2 ) // Delta eta(PV, pixel) cut
                                if( ( dPhi1 < 0 && dPhi2 < 0 ) || ( dPhi1 > 0 && dPhi2 > 0 ) ) // Delta phi sign cut
                                    if( cut > ddphi_left && cut < ddphi_right ) 
                                        L234.push_back(track((*c).X(), (*b).X(), (*a).X(), (*c).Y(), (*b).Y(), (*a).Y(), (*c).Z(), (*b).Z(), (*a).Z(), 3));
