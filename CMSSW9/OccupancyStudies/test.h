...
void test::StorePixelHit(int region){
    ...
    for(int a=0; a<bRecHitN; a++){
        ...
        TVector3 segTrack;
        //segTrack.SetXYZ( bRecHitGx->at(a), bRecHitGy->at(a), bRecHitGz->at(a) );
        segTrack.SetXYZ( bRecHitGx->at(a) - simVx->at(1), bRecHitGy->at(a) - simVy->at(1), bRecHitGz->at(a) - simVz->at(1) );
        Float_t segTrackEta = segTrack.Eta();
        
        Int_t regionFlag = 0;
        if( fabs(segTrackEta) <= 0.8 ) regionFlag = 1;
        if( fabs(segTrackEta) <= 1.4 && fabs(segTrackEta) > 0.8 ) regionFlag = 2;
        if( fabs(segTrackEta) <= 1.7 && fabs(segTrackEta) > 1.4 ) regionFlag = 3;
        if( fabs(segTrackEta) <= 2.1 && fabs(segTrackEta) > 1.7 ) regionFlag = 4;
        if( fabs(segTrackEta) <= 2.7 && fabs(segTrackEta) > 2.1 ) regionFlag = 5;
        if( fabs(segTrackEta) <= 3.0 && fabs(segTrackEta) > 2.7 ) regionFlag = 6;
        if( fabs(segTrackEta) > 3.0 ) continue;

        if( regionFlag != region ) continue;
    ...
    for(int a=0; a<fRecHitN; a++){
        ...
        TVector3 segTrack;
        //segTrack.SetXYZ( fRecHitGx->at(a), fRecHitGy->at(a), fRecHitGz->at(a) );
        segTrack.SetXYZ( fRecHitGx->at(a) - simVx->at(1), fRecHitGy->at(a) - simVy->at(1), fRecHitGz->at(a) - simVz->at(1) );
        Float_t segTrackEta = segTrack.Eta();
